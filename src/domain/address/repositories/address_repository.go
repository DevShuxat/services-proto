...
import (
	...
	repositories "github.com/DevShuxat/eater-service/src/infrastructure/repositories/address"
	"gorm.io/gorm"
)
...
func (r *NewAddressRepository) GetAddressById(ctx context.Context, id string) (*models.Address, error) {
	...
	err := r.db.WithContext(ctx).Table(tableAddress).First(&address).Error
	if err != nil {
...
func (r *NewAddressRepository) ListAddressByEaterId(ctx context.Context, eaterID string) (*models.Address, error) {
	var address models.Address
	result := r.db.WithContext(ctx).Table(tableAddress).First(&address, "eater_id = ?", eaterID)
	if result.Error != nil {
